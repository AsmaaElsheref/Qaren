...
class ContentList extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return ListView(
      ...
      children: [
        const AiSuggestionCard(suggestion: 'أنا قارن، للوصول إلى شارع الأمير سلطان بن سلمان، أرشح لك تطبيق ''أوبر لسرعة الاستجابة أو بولت إذا كنت تفضل السعر الأقل.',),
        ...
        SizedBox(
          ...
          child: ListView.separated(
            itemBuilder: (context, index) => GestureDetector(
              onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => TripDetails(serviceName: results[index].appName,),)),
              child: PriceResultCard(
                result: results[index],
                onBook: () {
                  Navigator.push(context, MaterialPageRoute(builder: (context) => TripDetails(serviceName: results[index].appName,),));
                },
              ),
            ),
